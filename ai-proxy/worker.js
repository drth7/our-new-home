  ...
  async fetch(request, env) {
    ...
    if (body && body.fetchUrl){
      ...
      const sys = 'You are modelling the furniture sold on a product page, using the attached product PHOTOS as ground truth (the first photo may show the whole staged set; later photos usually show individual pieces). The page text may be in any language. ' +
        ...
        'MODEL schema: {"name":string<=16,"w":metres,"d":metres,"h":metres,"color":"#hex","parts":[{"shape":"box"|"cylinder"|"sphere"|"cone"|"capsule","w":m,"h":m,"d":m,"r":m,"x":m,"y":m,"z":m,"rx":deg,"ry":deg,"rz":deg,"arc":deg,"sx":n,"sy":n,"sz":n,"color":"#hex"}]}. ' +
        'CURVED SHAPES — use them, most furniture is not boxy: "capsule" (r + total h) = soft cushions, padded seats/arms, rounded legs; cylinder with "arc" (20-360, e.g. 160-220) = an OPEN curved shell centred on +z (aim with ry) — perfect for barrel/curved chair backs, curved headboards, tub sofas; "sx"/"sy"/"sz" scale any part — oval tabletops (cylinder with sz 0.7), flattened cushions (sphere with sy 0.4). ' +
    ...
    if (body && body.generate){
      ...
      const sys = 'You output ONLY minified JSON (no prose, no markdown) describing a simple low-poly 3D furniture/prop model made of primitive parts. ' +
        'Schema: {"name":string<=16,"w":metres,"d":metres,"h":metres,"color":"#hex","parts":[{"shape":"box"|"cylinder"|"sphere"|"cone"|"capsule","w":m,"h":m,"d":m,"r":m,"x":m,"y":m,"z":m,"rx":deg,"ry":deg,"rz":deg,"arc":deg,"sx":n,"sy":n,"sz":n,"color":"#hex"}]}. ' +
        'CURVED SHAPES: "capsule" (r + total h) = soft cushions, padded arms, rounded legs; cylinder with "arc" (20-360) = an open curved shell centred on +z (aim with ry) for barrel chair backs, curved headboards, tub sofas; "sx"/"sy"/"sz" scale a part — oval tops (cylinder, sz 0.7), flattened cushions (sphere, sy 0.4). ' +
