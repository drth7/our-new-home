  ...
  async fetch(request, env) {
    ...
    if (body && body.fetchUrl){
      ...
      const sys = 'You are modelling the furniture sold on a product page, using ALL the attached product PHOTOS as ground truth — study every photo and cross-reference them: different angles reveal the base, the legs, the back and the details (the first photo may be a staged set; later ones usually show the piece alone). The page text may be in any language. ' +
        ...
        'FINISHES — this is what makes a model look REAL, always use them: add "metal":0.7-0.9 to anything metal/steel/chrome/brass (appliance frames, handles, legs, taps, lamp bases) and "rough":0.1-0.25 to glossy/lacquered/glass-like faces (dark appliance glass fronts, TV/oven screens, high-gloss doors); leave "rough" at its 0.85 default for matte wood, fabric, plaster and painted MDF. A stainless appliance = light-grey colour + metal 0.85; a black glass oven door = near-black colour + metal 0.4 + rough 0.12. ' +
        'DEPTH BY LAYERING — build detail by stacking THIN boxes front-to-back, never one flat face: a cabinet = a carcass box, then door/drawer panels standing ~0.02 m proud of the front, then handles ~0.01 m proud of those; a built-in appliance = a steel frame box, a dark glossy glass front just proud of it, a thin control strip, and a handle bar. This layering plus two or three finishes is exactly what separates a convincing model from a toy — spend parts on it. ' +
        'EXAMPLE — built-in oven cabinet 0.9w 0.62d 2.0h (origin centre-floor, front face at z +0.31): cream carcass {"shape":"box","w":0.9,"h":2.0,"d":0.62,"y":1.0,"color":"#e6e1d8"}; two lower doors {"shape":"box","w":0.42,"h":0.7,"d":0.02,"x":0.22,"y":0.42,"z":0.32,"color":"#e6e1d8"} (mirror at x -0.22) with slim handles {"shape":"box","w":0.026,"h":0.28,"d":0.03,"x":0.03,"y":0.42,"z":0.35,"metal":0.8,"color":"#b9bdc2"}; a stainless oven frame {"shape":"box","w":0.85,"h":0.7,"d":0.03,"y":1.15,"z":0.32,"metal":0.85,"rough":0.35,"color":"#b9bdc2"} + a black glossy glass door {"shape":"box","w":0.77,"h":0.46,"d":0.014,"y":1.1,"z":0.34,"metal":0.4,"rough":0.12,"color":"#0d0f13"} + a steel handle bar {"shape":"box","w":0.7,"h":0.024,"d":0.04,"y":1.4,"z":0.36,"metal":0.85,"color":"#b9bdc2"}; a microwave above it built the same way (steel frame + glass + a small control pad on the right). ' +
        ...
        'MATCH THE PHOTOS: silhouette, proportions, leg/arm/back style, and the EXACT material colours you see (two-tone finishes get two colours — e.g. a white-washed base with a wood top must not come out one flat colour). Use up to 24 parts when the shape needs them — spend them on layered panels, finishes and details; someone who owns the product should recognise it. ' +
    ...
    if (body && body.generate){
      ...
      const sys = 'You output ONLY minified JSON (no prose, no markdown) describing a simple low-poly 3D furniture/prop model made of primitive parts. ' +
        'Schema: {"name":string<=16,"kind":"chair"|"sofa"|"stool"|"bench"|"bed"|"table"|"desk"|"rug"|"storage"|"appliance"|"lamp"|"decor","seatH":m,"w":metres,"d":metres,"h":metres,"color":"#hex","parts":[{"shape":"box"|"cylinder"|"sphere"|"cone"|"capsule","w":m,"h":m,"d":m,"r":m,"x":m,"y":m,"z":m,"rx":deg,"ry":deg,"rz":deg,"arc":deg,"sx":n,"sy":n,"sz":n,"metal":0to1,"rough":0to1,"glass":true,"glow":true,"color":"#hex"}]}. ' +
        ...
        'FINISHES make it look real: add "metal":0.7-0.9 to metal/steel/chrome/brass parts (frames, handles, legs, lamp bases) and "rough":0.1-0.25 to glossy/glass-like faces (dark appliance fronts, screens, high-gloss doors); leave "rough" at its 0.85 default for matte wood/fabric/plaster. Build DEPTH by layering thin boxes: a cabinet or appliance = a body box + door/front panels ~0.02 m proud + handles proud of those; an appliance front = a steel frame + a dark glossy glass panel + a control strip. ' +
        'Rules: real-world sizes in metres; origin at the CENTRE of the floor footprint, y is UP, each part y is its centre height so the whole object rests on the floor (nothing below y=0); w/d/h are the overall bounding size; box uses w/h/d, cylinder/cone use r+h, sphere uses r; up to 20 parts — spend them on layered detail and finishes; keep it recognizable. ' +
        'COLOURS: give EVERY part its own realistic "color", and use 2-4 DIFFERENT colours across the object to show its separate materials/sections — never make the whole thing one flat colour. Examples: floor lamp = dark metal base (metal 0.8) + slim pole + warm cream shade; dining chair = wooden legs + fabric seat + cushion; plant = terracotta pot + green foliage; microwave = steel body (metal 0.85) + black glossy glass door (rough 0.12) + control pad. Pick tasteful, true-to-life hues. ' +
